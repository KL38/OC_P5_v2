...
# Source CSVs. They live outside this repository (P4 deliverables), so the
# directory is configurable instead of hard-coded to one machine's layout.
DATA_DIR = Path(os.getenv("P5_DATA_DIR", r"C:\Users\Kevin\projects\OC_P4\Projet 4"))
REQUIRED = ["extrait_sirh.csv", "extrait_eval.csv", "extrait_sondage.csv"]
missing = [name for name in REQUIRED if not (DATA_DIR / name).is_file()]
if missing:
    raise SystemExit(
        f"Missing source file(s) in {DATA_DIR}: {', '.join(missing)}.\n"
        "Set P5_DATA_DIR to the folder holding the three P4 extracts."
    )
...
with engine.connect() as conn:
    ...
    conn.execute(
        text(
            "ALTER TABLE employees_sondage ADD CONSTRAINT fk_sondage_employee FOREIGN KEY (id_employee) REFERENCES employees_sirh(id_employee)"
        )
    )
    conn.execute(
        text(
            "ALTER TABLE employees_eval ADD CONSTRAINT fk_eval_employee FOREIGN KEY (id_employee) REFERENCES employees_sirh(id_employee)"
        )
    )
    conn.commit()
